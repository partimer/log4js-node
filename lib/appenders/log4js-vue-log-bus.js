...
function VueLogBusAppender(layout, timezoneOffset) {
    ...
    const appender = (loggingEvent) => {
        ...
        if( loggingEvent.level.categoryName != 'default' ) {
            channelName = loggingEvent.level.categoryName+':'+channelName;
        }
      
        // emit the log message
        LogBus.$emit(channelName, `${layout(loggingEvent, timezoneOffset)}\n`);
    };
